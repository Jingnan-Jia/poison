...
def gradients(x_ori, ckpt_path_final, number, grad_label, return_logits = True, new=True):
    ...
    inputs:
        ...
        ckpt_path_final: where the model is saved.
    ...
    with tf.Session() as sess:
        saver.restore(sess, ckpt_path_final)  # restore model weights
...
def softmax_preds(images_ori, ckpt_path_final, return_logits=False):
    """
    Compute softmax activations (probabilities) with the model saved in the path
    specified as an argument
    :param images: a np array of images
    :param ckpt_path_final: a TF model checkpoint
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, ckpt_path_final)
        print('model is restored at: ', ckpt_path_final,'\n')
